...
const AreaChart: React.FC<AreaChartProps> = ({ data, column, color = "#EC4899" }) => {
  ...
  if (column.type === 'date') {
    ...
    return (
      <ResponsiveContainer width="100%" height={300}>
        <RechartsAreaChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: marginBottom }}>
          ...
          <Legend />
          <Area type="monotone" dataKey="value" name={column.name} stroke={color} fill={`${color}40`} />
        </RechartsAreaChart>
      </ResponsiveContainer>
    );
  }
